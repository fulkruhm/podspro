    ...
    <div className="space-y-6 pb-20 relative">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pr-12 md:pr-0">
        <div>
          <h2 className="text-2xl font-black text-slate-900 flex items-center">
            User Command Center
            <span className="ml-3 px-2 py-0.5 bg-red-600 text-white text-[9px] font-black rounded uppercase tracking-widest">SysAdmin Level</span>
          </h2>
          <p className="text-sm text-slate-500 font-medium">Global access control, security credential resets, and node provisioning.</p>
        </div>
      ...
      <div className="bg-blue-600 rounded-[1.5rem] p-5 text-white flex items-center justify-between shadow-xl shadow-blue-900/20">
        <div className="flex items-center">
          <div className="hidden sm:flex w-10 h-10 bg-white/20 rounded-xl items-center justify-center mr-4 text-xl">💡</div>
          <div>
            <h4 className="text-sm font-black uppercase tracking-wider">Administrative Controls</h4>
            <p className="text-[11px] opacity-90 font-medium mt-0.5">As System Admin, use <span className="font-bold underline">Unlock Node</span> to restore access for locked accounts (10 failed attempts).</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white p-6 rounded-[1.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Managed Nodes</div>
          <div className="text-3xl font-black text-slate-900">{users.length}</div>
        </div>
        <div className="bg-white p-6 rounded-[1.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Active Operations</div>
          <div className="text-3xl font-black text-green-600">{users.filter(u => u.status === 'active').length}</div>
        </div>
        <div className="bg-white p-6 rounded-[1.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Locked Accounts</div>
          <div className="text-3xl font-black text-red-600">{users.filter(u => u.isLocked).length}</div>
        </div>
      </div>

      {/* MOBILE CARD VIEW */}
      <div className="md:hidden space-y-4">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Node Management Cards</h3>
        {users.map((u) => (
          <div key={u.id} className="bg-white rounded-3xl border border-slate-200 shadow-lg overflow-hidden flex flex-col">
            <div className="p-5 flex items-start justify-between border-b border-slate-50 bg-slate-50/50">
              <div className="flex items-center space-x-3">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-white text-lg ${u.isLocked ? 'bg-red-600' : 'bg-slate-900'}`}>
              ...
              <div className="flex flex-col items-end gap-1">
                <button 
                  ...
                  className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                    ...
                    u.status === 'paused' ? 'bg-amber-100 text-amber-700 border-amber-200' :
                ...
                {u.isLocked && (
                  <span className="text-[8px] font-black text-red-600 uppercase">SYSTEM LOCKED</span>
                )}
              </div>
            </div>
            ...
            <div className="px-5 py-4 space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Access Role</span>
                <span className="text-xs font-black text-blue-600 uppercase tracking-widest">{u.role}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Node Scope</span>
                <span className="text-[10px] text-slate-500 font-bold uppercase">{u.assignedStore || 'Global Network'}</span>
              </div>
            </div>
          ...
          <table className="w-full text-left min-w-[800px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Access Scope</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Operational Status</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">System Controls</th>
              </tr>
            </thead>
                ...
                <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-5">
                    <div className="flex items-center space-x-4">
                      <div className={`w-11 h-11 rounded-2xl flex items-center justify-center font-black text-white shadow-lg transition-transform group-hover:scale-105 ${u.isLocked ? 'bg-red-600' : 'bg-slate-900'}`}>
                        {u.isLocked ? '🔒' : u.name.charAt(0)}
                      </div>
                      <div>
                        <div className="text-sm font-black text-slate-900">{u.name}</div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tight flex items-center">
                          @{u.username}
                          {u.isLocked && <span className="ml-2 text-[8px] text-red-600 font-black border border-red-200 px-1 rounded">LOCKED</span>}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      <span className="text-xs font-black text-blue-600 uppercase tracking-widest">{u.role}</span>
                      <span className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">{u.assignedStore || 'Global Network'}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <button 
                      ...
                      className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all active:scale-95 ${
                        u.status === 'active' ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' :
                        u.status === 'paused' ? 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100' :
                  ...
                  <td className="px-8 py-5 text-right">
                    <div className="flex justify-end items-center space-x-3">
                      ...
                      {u.id !== currentSysAdminId ? (
                        ...
                        <span className="text-[9px] font-black text-slate-300 uppercase italic tracking-widest mr-4">Current Session</span>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-4xl h-[80vh] rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <div>
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">System Audit Trail</h3>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Real-time security & operation logging</p>
              </div>
              <button onClick={() => setShowLogsView(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
              ...
              <div className="space-y-3">
                {auditLogs.length === 0 ? (
                  <div className="text-center py-20">
                    ...
                    <p className="text-slate-400 font-bold uppercase text-xs tracking-widest">No audit entries found</p>
                  </div>
                ) : (
                  auditLogs.map((log) => (
                    <div key={log.id} className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex items-start space-x-4 hover:bg-slate-100/50 transition-colors">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start mb-1">
                          <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${
                            ...
                            log.category === 'provisioning' ? 'bg-purple-100 text-purple-700' :
                        ...
                        <div className="text-[11px] text-slate-600 font-medium mb-2">{log.details}</div>
                        <div className="text-[9px] font-bold text-slate-400 uppercase tracking-tight">
                          Initiated by: <span className="text-slate-900">{log.userName}</span> (@{users.find(u => u.id === log.userId)?.username || 'unknown'})
                        </div>
                      </div>
                    </div>
                  ))
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 md:p-10">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Provision New Node</h3>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Security Clearance: Level 4</p>
                </div>
              ...
              <form onSubmit={handleAddSubmit} className="space-y-5">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Initial Pass-Token (Optional)</label>
                  <input 
                    type="password" 
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 md:p-10">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Reset Pass-Token</h3>
                <button onClick={() => setResetPassUser(null)} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
              ...
              <div className="mb-8 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                <p className="text-xs text-blue-700 font-bold leading-relaxed">
                  You are resetting credentials for <span className="font-black">@{resetPassUser.username}</span>. 
                  This action will be logged in the security audit trail.
                </p>
              </div>

              <form onSubmit={handleResetPasswordSubmit} className="space-y-6">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">New Pass-Token</label>
                  <input 
                    type="password" 
                    required
                    autoFocus
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border border-red-100 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 md:p-10">
              ...
              <div className="text-center mb-8">
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Critical Action</h3>
                <p className="text-sm text-slate-500 font-medium mt-2">
                  Are you sure you want to permanently delete <span className="font-black text-slate-900">@{deleteConfirmUser.username}</span>? 
